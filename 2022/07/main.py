...
for line in load_input().split('$'):
    ...
    if line == 'cd ..':
        ...
        current_dir = Directory(line.split(' ')[-1], current_dir)
        directories.append(current_dir)
    if line.startswith('ls'):
        for file in line.split('\n')[1:]:
            try:
                current_dir.size += int(file.split(' ')[0])
                parent_dir = current_dir.parent_directory
                while parent_dir.name is not None:
                    parent_dir.size += int(file.split(' ')[0])
...
print(sum(sizes))
print("--- Part Two ---")
sizes = sorted([dir.size for dir in directories])
available_space = DISK_SPACE-sizes[-1]
required_space = 30000000 - available_space
sizes = sorted([dir.size for dir in directories if dir.size > required_space])
print(sizes[0])